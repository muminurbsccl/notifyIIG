...
export const upstreamBackhaulAdapter: WorkbookSheetAdapter = {
  ...
  parse(sheet, businessDate): SheetAdapterResult {
    ...
    for (let index = headerIndex + 1; index < sheet.rows.length; index += 1) {
      ...
      const provider = resolveCanonicalProvider(heading); if (!provider) issues.push({ code: "MISSING_PROVIDER", severity: "error", message: "Backhaul row has no provider section", source });
      const providerId = cellText(row[1]); const bscplcId = cellText(row[2]); const primary = bscplcId || providerId;
      if (!primary) issues.push({ code: "MISSING_IDENTIFIER", severity: "error", message: "Backhaul row has no durable identifier", source });
