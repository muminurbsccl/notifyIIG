...
export const internetExchangeAdapter: WorkbookSheetAdapter = {
  ...
  parse(sheet, businessDate): SheetAdapterResult {
    ...
    const headerIndex = sheet.rows.findIndex((row) => {
      ...
      return keys.includes("circuit id") && keys.some((key) => key === "permission expiry" || key === "permission expiration");
    });
    ...
    const indexes = {
      serial: column(headers, "sl", "serial", "serial no"), customer: column(headers, "customer link id", "customer id", "link id"),
      circuit: column(headers, "circuit id"), provider: column(headers, "provider name", "provider"), service: column(headers, "service type", "service"),
      ...
      permissionExpiry: column(headers, "permission expiry", "permission expiration"), procedure: column(headers, "procedure start", "renewal procedure start", "renewal procedure start date"),
      cost: column(headers, "monthly cost", "cost"), remark: column(headers, "remark", "remarks", "notes"),
    };
    ...
    for (let index = headerIndex + 1; index < sheet.rows.length; index += 1) {
      ...
      const customer = value(row, indexes.customer); if (customer) identifiers.push(importIdentifier("customer_link", customer, false));
      ...
      circuitCandidates.push({
        ...
        location: null, segment: null, connectedRouter: null, startDate, expiryDate, renewalProcedureStartDate,
        monthlyCost: cost.monthlyCost, currency: cost.currency, rawCostDetails: cost.rawDetails,
